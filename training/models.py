...
def ViT_small_patch16_224(pretrained, **kwargs):
    ...
    if pretrained:
        url = URLs['ViT_small_patch16_224']
...
def ViT_base_patch16_224(pretrained, **kwargs):
    model = VisionTransformer(
        patch_size=16,
        embed_dim=768,
        depth=12,
...
def ViT_large_patch16_224(pretrained, **kwargs):
    model = VisionTransformer(
        patch_size=16,
        embed_dim=1024,
        depth=24,
    ...
    if pretrained:
        url = URLs['ViT_large_patch16_224']
        weight_path = get_weights_path_from_url(url)
        param = paddle.load(weight_path)
        model.set_dict(param)
